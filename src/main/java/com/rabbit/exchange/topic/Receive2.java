...
public class Receive2 {
    public static void main(String[] args) {
        ...
        try {
            ...
            channel.queueDeclare("topicQueue02",true,false,false,null);//创建队列
            channel.exchangeDeclare("topicExchange", "topic", true);                    //创建交换机
            //监听某个队列并获取队列中的数据，由于是fanout类型的交换器所以不需要使用RoutingKey进行绑定
            channel.queueBind("topicQueue02", "topicExchange", "aa.*");     //创建bind连接
            ...
            channel.basicConsume("topicQueue02", true, "", new DefaultConsumer(channel) {
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                    ...
                    System.out.println("topic交换机2*处理的消息：" + message);
                }
            });
        } catch (IOException e) {
            e.printStackTrace();
